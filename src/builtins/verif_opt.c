...
void	ft_print_usage_nuoh(char cmd, char opt)
{
	if (cmd == 'n')
	{
		miniprintf(STDERR_FILENO,
			"21sh: unsetenv: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: unsetenv [name]\n");
	}
	else if (cmd == 'u')
	{
		miniprintf(STDERR_FILENO, "21sh: unset: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: unset [name]\n");
	}
	else if (cmd == 'o')
	{
		miniprintf(STDERR_FILENO, "21sh: echo: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: echo [-n] [string...]\n");
	}
	else if (cmd == 'h')
	{
		miniprintf(STDERR_FILENO, "21sh: history: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: history\n");
	}
	else if (cmd == 'x')
		miniprintf(STDERR_FILENO,
			"21sh: exit: illegal option -- %c\nusage: exit [n]\n", opt);
}
...
int		ft_print_usage(char cmd, char opt)
{
	if (cmd == 'c')
	{
		miniprintf(STDERR_FILENO, "21sh: cd: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: cd [directory] or cd - or cd ~\n");
	}
	else if (cmd == 'v')
	{
		miniprintf(STDERR_FILENO, "21sh: env: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: env [-i] [name=value]\n");
	}
	else if (cmd == 'e')
	{
		miniprintf(STDERR_FILENO, "21sh: export: illegal option -- %c\n", opt);
		miniprintf(STDERR_FILENO, "usage: export name[=word] or export -p\n");
	}
	else if (cmd == 's')
	{
		miniprintf(STDERR_FILENO, "21sh: setenv: illegal option -- %c\n", opt);
