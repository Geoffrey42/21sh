...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/07 18:23:59 by piranucc          #+#    #+#             */
/*   Updated: 2017/04/20 03:01:51 by piranucc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_file_out(char **tabl, int *fd, int i, int flag)
{
	...
	{
		if (errno == EISDIR)
			miniprintf(STDERR_FILENO, "21sh %s: Is a directory\n", tabl[i + 1]);
		...
		else if (access(tabl[i + 1], X_OK) == -1)
		{
			...
			if (errno == EACCES)
				miniprintf(STDERR_FILENO,
					"21sh: %s: permission denied\n", tabl[i + 1]);
			else
				miniprintf(STDERR_FILENO,
					"21sh: %s: Is a directory\n", tabl[i + 1]);
			closedir(rep);
		}
		close(*fd);
		exit(F);
